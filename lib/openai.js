...
function buildSystemPrompt(analysisData, contextSummary) {
  const contextLine = contextSummary
    ? `\nCURRENT VIEW CONTEXT: ${contextSummary}\n`
    : '';

  return `You are the AI advisor built into LinkLead, a link tracking and lead-generation analytics tool. You are always present as a side panel advisor — like Supabase's AI assistant — and you know exactly what the user is looking at.${contextLine}

ANALYSIS DATA (pre-computed by the local analytics engine, zero API cost):
${JSON.stringify(analysisData, null, 2)}

GUIDELINES:
- Be concise and actionable. Users want quick, specific insights they can act on.
- You know the current page context (link, category, or dashboard). Scope your answers accordingly.
- Use data from the analysis above to support every claim. Never make up numbers.
...
- Keep responses under 280 words unless the user asks for detail.
 ...
 */
export async function chatWithContext(message, analysisData, chatHistory = [], contextSummary = null) {
  ...
  const messages = [
    { role: 'system', content: buildSystemPrompt(analysisData, contextSummary) },
