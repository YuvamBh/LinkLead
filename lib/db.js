...
// Returns all unique categories present across all links
...
// Pass a slug to filter to a single link, or categoryFilter for a category view.
export async function getStats(slugFilter = null, categoryFilter = null) {
  const [allClicks, allLinks] = await Promise.all([getClicks(), getLinks()]);

  // Lazy-migrate any links that have no category yet
  let migrated = false;
  allLinks.forEach(link => {
    if (!link.category) {
      const classification = classifyLink(link.destination);
      link.category = classification.category;
      link.categoryConfidence = classification.confidence;
      link.tags = classification.tags || [];
      migrated = true;
    }
  });
  if (migrated) await writeList(LINKS_KEY, LINKS_FILE, allLinks);

  // Determine which slugs are in scope for this query
  ...
  let links = scopedLinks;
  let clicks = slugFilter
